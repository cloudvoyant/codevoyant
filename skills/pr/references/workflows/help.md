...
| `/pr merge [pr-id] [--squash\|--rebase\|--merge] [--delete-branch] [--cleanup] [--subject "..."] [--body "..."] [--no-watch-ci] [--yes]` | Merge the PR/MR (squash by default, semantic-release aware); then watches post-merge CI on the base branch and notifies on failure (opt out with `--no-watch-ci`). `--cleanup` deletes the merged source branch locally and remotely after a clean merge (alias `/pr land`) |
