...
- **`light`** — fast and low-cost. For mechanical and validation agents: review passes, freshness checks, triage.
- **`standard`** — default reasoning. For most analysis agents: market/competitive/user-problem researchers, internal researchers, planners.
...
- In an agent file using the header format: a `**Model tier:** standard` line instead of `**Model:** <id>`.
...
This file is mirrored per published skill package (`spec`, `pr`, `skill`, `plan`, `pm`, `explore`, `ed`). Edit all copies together, keeping each skill's usage examples local.
