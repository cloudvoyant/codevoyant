...
Store these as `SKILL_READ_BASELINE` and union them into the allow set alongside `DEV_ALLOW`.

Store the resulting list as `DEV_ALLOW`.

## Step 3: Merge into settings.json

Read the existing settings (or start from `{}` if absent), union `DEV_ALLOW` and `SKILL_READ_BASELINE` into `.permissions.allow` (deduplicate, sort), and write the file back. Use the Edit tool or `jq` for the merge.
