...
- **`light`** — fast and low-cost. For mechanical and validation agents: review passes, validation rounds, estimation checks.
- **`standard`** — default reasoning. For most analysis agents: codebase architecture scans, Linear context agents, plan drafters.
...
- In an agent file using the header format: a `**Model tier:** light` line instead of `**Model:** <id>`.
