...
- **OpenCode** loads skills through the `skill` tool, which accepts only a `name` — arguments typed after the command are dropped unless a custom command file exists at `~/.config/opencode/command/{skill}.md` containing a `$ARGUMENTS` placeholder. OpenCode substitutes the typed text for `$ARGUMENTS` only inside command templates — it does NOT substitute `$ARGUMENTS` or `$N` inside a SKILL.md body, so a literal token there reaches the model unexpanded. Codevoyant ships the shared wrapper template (`skills/shared/opencode-command/template.md`, vendored to each skill's `references/opencode-command/`) and an optional installer (`scripts/install-opencode.sh`) for this.
...
2. **Never embed `$ARGUMENTS` or `$N` substitution tokens in a SKILL.md body.** Claude Code substitutes them natively; OpenCode does NOT substitute them inside skill content (only inside command templates), so a literal token there reaches the model unexpanded or is silently dropped. Skill bodies parse the invocation as plain text instead. The only place `$ARGUMENTS` appears is the opencode command wrapper template (`skills/shared/opencode-command/template.md`), where both platforms substitute it.
...
Read the invocation from the current request. VERB = first non-flag argument; REMAINING_ARGS = everything after VERB (order and flags preserved). The invocation may arrive inline (Claude Code), via an opencode `$ARGUMENTS` command wrapper, or as a plain message.
