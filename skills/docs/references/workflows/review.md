...
- **Missing/empty globs** (Rule 1): a doc has no leading `globs:` frontmatter, or its `globs:` list is empty. `current_text`: "(no globs frontmatter)"; `replacement_text`: the `globs:` frontmatter stub from the template with a `<!-- TODO -->`; `rationale`: "coverage-and-api Rule 1: every doc declares the paths it covers."
